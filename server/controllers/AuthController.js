...
export const generateToken = (req, res, next) => {
  try {
    ...
    if (appID && serverSecret && userId) {
      ...
      return res.status(200).json({ token }); // Send response here
    }
    return res.status(400).send("User id, app id and server secret are required"); // Move the response inside the else block
  } catch (err) {
    console.log({ err });
    next(err);
  }
